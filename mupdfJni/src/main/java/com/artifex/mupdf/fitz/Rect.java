...
public class Rect
{
	...
	public int getRange(float x,float y){
		return (int) Math.hypot((centerX()-x),(centerY() - y));
	}
	...
	public boolean contain(RectF rectF){
		if(x0 < rectF.left && x1 < rectF.left) return false;
		if(x0 > rectF.right && x1 > rectF.right) return false;
		if(y0 < rectF.top && y1 < rectF.top) return false;
		if(y0 > rectF.bottom && y1 > rectF.bottom) return false;
