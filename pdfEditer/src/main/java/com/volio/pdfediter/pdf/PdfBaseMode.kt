...
abstract class PdfBaseMode (val context: Context,val callback: PdfPageCallback){
    ...
    private var spaceTouchSelect:Float = 100f
            ...
            override fun onLongPress(e: MotionEvent?) {
                ...
                e?.let {
                    ...
                    for (item in listDraw){
                        if (item.getRectDraw().contains(e.x,e.y)&&item.checkPointText(e.x,e.y)){
    ...
    protected fun moveTouchEvent(event: MotionEvent,onMove:()->Unit,onScale:()->Unit){
        when{
            ...
            typeTouch == PdfView.TYPE_SELECT->{
                ...
                currentPage?.changePointSelect(fixedPointSelect.x,fixedPointSelect.y,event.x,event.y)
                currentPage?.let {
